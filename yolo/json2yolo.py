...
def convert_coco_json(json_path="../coco/annotations/", use_segments=False, cls91to80=False):
    ...
    for json_file in [Path(json_path)]:
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f"Annotations {json_file}"):
            ...
            bboxes = []
            segments = []
            # 额外写个函数做一下check，标注文件中的格式为下面这样，第一个是类别不用管，然后是将图片长宽归一后的xmin, ymin位置，后面是
            # 0
            # 0.564997
            # 0.400819
            # 0.480499
...
if __name__ == "__main__":
    convert_coco_json(
        "./table_det/cropped.json",  # directory with *.json
        use_segments=True,
        cls91to80=False,
    )
